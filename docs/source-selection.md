...
| --- | --- | --- | --- |
| [Filmpalast](https://filmpalast.to/) | HTML title search at `/search/title/<title>`. Sampled movie/episode details supplied title, year and explicit `SxxExx`, without a public IMDb ID. | Root, search and details returned HTTP 200. VOE resolution produced HLS; an exact Game of Thrones episode also yielded an MPEG-TS segment in a bounded check. | Included for movies and exact series episodes through supported VOE mirrors. |
| [Filmo](https://filmo.to/) | JSON `/search/suggest?q=<title>` returns movie titles and detail URLs; year is verified on the detail page. | A fresh cookie/CSRF session and `POST /n` produce a token redirect to VOE. Public media checks succeeded, but a cookie-free token request returned 404. | Included for movies on Android Mobile/Desktop runtimes that honor manual redirects, with a capability check before session creation. The inspected iOS/TV/Smart TV bridges do not support this path. Series support was not established. |
